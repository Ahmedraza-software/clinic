...
class Command(BaseCommand):
    help = 'Import first 50 patients from Google Sheets into database'

    def handle(self, *args, **options):
        sheet_id = '1fYUdK_cprGPj8UEoZVI38DiqI7P4G_loOXf8Xg62TWc'
        sheet_name = 'Sheet1'
        
        self.stdout.write(self.style.SUCCESS(f'Importing first 50 patients from Google Sheet: {sheet_id}'))
        ...
        try:
            ...
            # Only process first 51 lines (header + 50 patients)
            sample_lines = lines[:51]
            sample_text = '\n'.join(sample_lines)
    ...
    def import_patients_to_db(self, patients):
        ...
        for patient_data in patients:
            try:
                ...
                imported_count += 1
                self.stdout.write(self.style.SUCCESS(f'✓ Imported: {patient_data["first_name"]} {patient_data["last_name"]} ({email})'))
                
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'✗ Error importing {patient_data.get("first_name", "unknown")}: {e}'))
                continue
